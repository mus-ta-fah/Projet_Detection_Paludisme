...
export default function AnalyzePage() {
  ...
  const handleExportPDF = () => {
    ...
    // Add logo/header
    doc.setFontSize(20);
    doc.text('Malaria Detection Report', 105, 20, { align: 'center' });
    ...
    doc.setFontSize(12);
    doc.text(`Prediction: ${predictionResult?.prediction}`, 30, 70);
    doc.text(`Confidence: ${formatPercentage(predictionResult?.confidence || 0)}`, 30, 80);
    doc.text(`Model: ${predictionResult?.model_name}`, 30, 90);

    // Add probabilities
    doc.text('Detailed Probabilities:', 20, 110);
    doc.text(`Parasitized: ${formatPercentage(predictionResult?.probability_parasitized || 0)}`, 30, 120);
    doc.text(`Uninfected: ${formatPercentage(predictionResult?.probability_uninfected || 0)}`, 30, 130);

    // Save
    doc.save('malaria-analysis-report.pdf');
          ...
          <CardContent className="space-y-4">
                ...
                <SelectContent>
                  {modelsData?.models?.map((model: ModelInfo) => (
                    <SelectItem key={model.id} value={model.id}>
                      {model.name} - {formatPercentage(model.accuracy * 100)} accuracy
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div
              className={cn(
                ...
                selectedFile ? 'border-primary bg-primary/5' : 'border-muted-foreground/25 hover:border-primary'
              )}
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant={result.result.is_parasitized ? 'destructive' : 'default'}
                ...
                <div className="space-y-2">
                  ...
                  <Progress value={predictionResult?.confidence || 0} className="h-3" />
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-medium">
                        {formatPercentage(predictionResult?.probability_parasitized || 0)}
                      </span>
                    </div>
                    <Progress
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-medium">
                        {formatPercentage(predictionResult?.probability_uninfected || 0)}
                      </span>
                    </div>
                    <Progress
                ...
                <div className="space-y-2 border-t pt-4">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">{predictionResult?.model_name}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">Processing Time</span>
                    <span className="font-medium">{predictionResult?.inference_time_ms?.toFixed(0)}ms</span>
                  </div>
                </div>
